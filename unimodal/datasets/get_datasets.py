...
def get_datasets(args):
    if args.in_dataset == 'ImageNet':
        if args.method in ['msp', 'energy', 'maxlogits','cadref']:
            with open('./configs/msp.yaml', 'r') as file:
                cfg = yaml.load(file, Loader=yaml.SafeLoader)
        if args.method in ['optfs', 'react']:
            with open('./configs/OptFS.yaml', 'r') as file:
                cfg = yaml.load(file, Loader=yaml.SafeLoader)
        if args.method in ['ash']:
            with open('./configs/ash.yaml', 'r') as file:
                cfg = yaml.load(file, Loader=yaml.SafeLoader)
    ...
    return train_loader, ood_loaders, cfg
